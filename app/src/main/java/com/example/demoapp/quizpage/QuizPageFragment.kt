...
import com.example.demoapp.databinding.FragmentQuizPageBinding
...
class QuizPageFragment : Fragment() {
    ...
    override fun onCreateView( inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle? ): View? {
        ...
        val binding = DataBindingUtil.inflate<FragmentQuizPageBinding>(inflater, R.layout.fragment_quiz_page, container, false)

        //Associating the view Model
        viewModel = ViewModelProviders.of(this).get(QuizPageViewModel::class.java)
        ...
        binding.submitButton.setOnClickListener { _ : View ->
            ...
            if (-1 != checkedId) {
                ...
                if (tempQuestionIndex < numQuestions) {
                    //currentQuestion = questions[viewModel.questionIndex.value?:0]
        ...
        (activity as AppCompatActivity).supportActionBar?.title = getString(R.string.db_quiz, viewModel.questionIndex.value!!.plus(1), numQuestions)
        return binding.root
    }

    private fun completeQuiz(){
       val action = QuizPageFragmentDirections.actionGameToScore()
